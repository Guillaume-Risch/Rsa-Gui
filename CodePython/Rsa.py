...
def RSA ( ):
    tabReste,tabDiviseur,tabDividende,tabQuotient = genereTabs()
    alphabet = genereAlphabet()
    n,phi,e,p,q = initialisation(tabNombrePremier,tabReste,tabDiviseur,tabDividende,tabQuotient) # on initialise les variables importantes pour le RSA 
    while tabDividende[len(tabDividende)-1] * 1 + tabDiviseur[len(tabDiviseur)-1] * (-1) != 1: #fix erreur coeff bezout
        tabReste = []
        tabDiviseur = []
        tabDividende = []
        tabQuotient = []
        n,phi,e,p,q = initialisation(tabNombrePremier,tabReste,tabDiviseur,tabDividende,tabQuotient)
    u,v = Bezout(tabDividende[len(tabDividende)-1],tabDiviseur[len(tabDiviseur)-1],1,-1,len(tabDividende)-1,tabReste,tabDiviseur,tabDividende,tabQuotient) # calcul coeff de bezout 
    d : int = isKeyNegative(v,phi) # on vérifie l'intégrité de la clé privée c'est-à-dire qu'elle ne soit pas < 0 cf commentaires de la fonction isKeyNegative
    motachiffrer : str = input("Saisir un mot : ") # on demande à l'utilisateur de saisir un mot au clavier (majuscule uniquement pour le moment, rajouter les minuscules)
    tabValChiffre = []
    chiffrage(motachiffrer,tabValChiffre,alphabet,e,n) # on met dans tabValChiffre les valeurs correspondantes à chaque lettre chiffré par l'algo 
    motdechiffree : str
    motdechiffree = dechiffrage(tabValChiffre,alphabet,d,n) #on récupère chaque valeur du tableau tabValChiffre et on déchiffre son contenu pour ensuite récupérer les valeurs déchiffrés
                                                            # et ensuite concatener les lettres obtenues grâce à la fonction findIntToChar
    return (motdechiffree,n,e,tabValChiffre)
...
mot, n, e, tabValChiffre = RSA()
print("Le mot était : ", mot )
#####################################

print("partie crackage")

def crackageRSA (n,e,tabNombrePremier,tabValChiffre):
    ...
    return motdechiffrecrack

print("Le mot chiffré était : ",crackageRSA(n,e,tabNombrePremier,tabValChiffre))
